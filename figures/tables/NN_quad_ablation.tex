\begin{table*}[h!]
    ...
    \end{tabular}
    \caption{
    Results of the ablation study measured in term of MAE on the $\boldsymbol{r_q}$, $\boldsymbol{u}$ and $\boldsymbol{r_u}$ components of the output vector. $V, A$ refer to the different inputs considered (linear and angular velocities, accelerations respectively). The presence of one as a subscript to the \textbf{GRU} model means that it is used as an input (e.g. $\boldsymbol{GRU_{A}}$ takes as input the acceleration only).}
     \label{tab:ablation}
\end{table*}