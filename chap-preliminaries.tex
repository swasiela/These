...
The system dynamic can be described using the following set of ordinary differential equations (\myglsentry{odes}):
...
Under this consideration and according to Newton's second law, the total force ($\boldsymbol{f_{tot}}$) and torque ($\boldsymbol{\tau_{tot}}$) acting on the quadrotor can be expressed by taking an additional fictitious force due to the displaced CoM in $F_B$ s.t. 
...
This controller takes advantage of the well-known quadrotor flat outputs ~\cite{cFlat} to track a desired trajectory defined as $\q_d(t) = [\boldsymbol{x}_d(t) \, \boldsymbol{v}_d(t) \, \boldsymbol{a}_d(t) \, \psi_d(t) \, \Dot{\psi}_d(t)] \in \mathbb{R}^{11}$ respectively composed of the desired linear positions, velocities, accelerations, yaw orientation angle, and yaw angular velocity.
...
Finally, one can compute the control input vector $\u$ by mean of the same standard allocation matrix $\boldsymbol{T}$ of Equation~\ref{eq:alloc_mat} using nominal parameters denoted $\boldsymbol{T}_n$ s.t.:
...
The trajectory generation problem is solved independently for each component of $\boldsymbol{\gamma}$, and, in order to generate smooth trajectories in a global context, ensure the continuity of their derivatives up to the jerk (acceleration derivative). 
Note that even if the yaw angular acceleration is not required by the control law of Section~\ref{sec:quad_model}, it is used to ensure the smoothness of the trajectory. 
As for both initial and goal jerk, they are set to zero to facilitate a smooth transition between trajectories.
...
This also implied that the time spend to reach such velocity as to be minimized, meaning that the time spent at maximum accelerations during transient phases as to be maximized.
The same apply to the jerk and snap derivatives, i.e. during acceleration variations the maximum achievable jerk must be maintained as long as possible, and the duration of jerk variation phases must be minimized.
...
By doing so, the duration of these phases is minimized, and gradually the maximum speed phase is maximized.
The principle of this trajectory generation is illustrated by Figure~\ref{fig:kino}.

\begin{figure} [t]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/models/kino.png} 
...
\begin{figure} [t]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/models/unicycle.png} 
...
The robot state is defined as $\boldsymbol{q} = [x  \, y \, \theta]$ where $(x \, y)$ are the planar position in $F_W$ and $\theta$ is the body heading (see Figure~\ref{fig:unicycle}).

Define the control input vector $\u = [\omega_r \, \omega_l] \in \mathbb{R}^2$, where $(\omega_r, \, \omega_l)$ are respectively the right and left wheel angular velocity.
