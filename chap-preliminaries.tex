...
Also, consider that the center of mass (\myglsentry{com}) is displaced from the robot geometric center of an offset $\boldsymbol{x_{c}} = [x_{cx} \, x_{cy} \, x_{cz}]$ expressed in $F_B$ as depicted in Figure.~\ref{fig:quad}. 
...
\begin{equation}\label{eq:quad_dynamic}
   ...
   \right .
\end{equation}
In line with Section~\ref{sec:sensi}, let the vector $\p = [m \, x_{cx} \, x_{cy} \, x_{cz} \, J_{x} \, J_{y} \,J_{z} \, k_f \, k_{\tau}] \in \mathbb{R}^{9}$ represent all the parameters used in the robot model outlined above that are subject to uncertainty. 
Note that this vector can be reduced according to the different scenarios proposed in this thesis.
...
This controller takes advantage of the well-known quadrotor flat outputs ~\cite{cFlat} to track a desired trajectory defined as $\q_d(t) = [\boldsymbol{x}_d(t) \, \boldsymbol{v}_d(t) \, \boldsymbol{a}_d(t) \, \psi_d(t) \, \Dot{\psi}_d(t)] \in \mathbb{R}^{11}$ respectively composed of the desired positions, velocities, accelerations, yaw orientation angle, and yaw angular velocity.
Let the linear position error and linear velocity error
\begin{equation}\label{eq:pos_error}
  \boldsymbol{e_x}(t) = \boldsymbol{x}(t) - \boldsymbol{x}_d(a, t) \in \mathbb{R}^3, \,
  \boldsymbol{e_v}(t) = \boldsymbol{v}(t) - \boldsymbol{v}_d(a, t) \in \mathbb{R}^3,
\end{equation}
...
This control strategy begins by computing the desired basis vector for the third body-fixed frame
\begin{equation}
...
This vector is then used to compute the second and first body-fixed frame basis vector:
\begin{equation}
...
\end{equation}
From the three body-fixed frame basis vector it is then possible to define the desired rotation matrix $\boldsymbol{R}_d(\psi_d, t) = [\boldsymbol{r}_{1d} \, \boldsymbol{r}_{2d} \, \boldsymbol{r}_{3d}]$, and to compute the following attitude error:
...
\end{equation}
Finally, to simplify the overall controller design, the desired yaw rate $\Dot{\psi}_d$ is always set to 0 in this thesis, allowing the angular velocity error to be expressed as:
\begin{equation}\label{eq:rate_error}
  \boldsymbol{e_\omega}(t) = \omega(t) \in \mathbb{R}^3.
\end{equation}

According to the aforementioned errors, this control strategy computes the desired thrust $f_d(t)$ and desired torques $\boldsymbol{\tau}_d(t)$ that allow the tracking of the desired trajectory $\q_d(t)$:
\begin{equation}\label{eq:desftau}
  \left \{
    \begin{array}{l l}
      f_d(t) &= \left( -\boldsymbol{K_x} \cdot \boldsymbol{e_x} - \boldsymbol{K_v} \cdot \boldsymbol{e_v} + m(g Z_W + \boldsymbol{a}_d) \right)^T \cdot (R(\rho) \cdot Z_W) \\
      \boldsymbol{\tau}_d(t) &= -\boldsymbol{K_R} \cdot \boldsymbol{e_R} - \boldsymbol{K_\omega} \cdot \boldsymbol{e_\omega}
    \end{array}
  \right .
\end{equation}
Note that the choice of setting the desired yaw rate to zero cancels terms in the expression of $\boldsymbol{\tau}_d$ compared to the original expression in ~\cite{cLee}.

Finally, one can compute the control input vector $\u$ by mean of the same standard allocation matrix $\boldsymbol{T}$ of Equation~\ref{eq:alloc_mat} under nominal parameters denoted $\boldsymbol{T}_n$ s.t.:
\begin{equation}\label{eq:control_lee}
  \u = \boldsymbol{T}_n^{-1}\begin{bmatrix}
      ...
      \end{bmatrix}.
\end{equation}
Note that under nominal case, $\boldsymbol{T} = \boldsymbol{T}_n$; thus, the propeller thrust and torques applied to the real system, as for $f$ and $\boldsymbol{\tau}$ in Equation~\ref{eq:quad_newton}, perfectly match the desired thrust and torques computed by the control law.

\subsubsection{State interpolation}

With the quadrotor model and controller now defined, this section presents the method (hereafter referred to as the \emph{steering} method) for computing a desired trajectory that the geometric controller will track.
Given a quadrotor initial desired state and a goal desired state
