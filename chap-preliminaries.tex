\chapter{Definitions and Systems Overview}\label{chap:models}
\markboth{Definitions and Systems Overview}{}% To set left/right header
% \localtableofcontents

This chapter first introduces the notion of \emph{closed-loop sensitivity}, which forms a key foundation of this thesis.
This concept quantifies how variations of some models parameters (supposed to be uncertain) affect the evolution of the system in closed-loop, i.e., by also taking into account any controller chosen for executing the task.
...
The system dynamic can be described using the following set of \myglsentry{odes}:
...
\begin{figure} [t]
  \centering
  \includegraphics[width=0.7\linewidth]{figures/models/radius.png} 
...
Once the path is computed, the x and y components are discretized using a given time step, and then differentiated to access the corresponding desired velocities and accelerations required by Equation~\ref{eq:unic_control_law}.
