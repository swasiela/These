...
As highlighted in Chapter~\ref{chap:samp}, the computation of such tubes using \myglsentry{odes} considerably reduces the efficiency of sampling-based methods, particularly for complex systems.
...
To demonstrate the reliability and generalizability of the learned model, a test set of 1.000 trajectories was also generated, but with a velocity magnitude of $v_{magn} = 1.1 m.s^{-1}$ for the Dubins curve parametrization. 
Consequently, the test set includes trajectories with higher velocities than those in the validation set.
...
\subsection{Model comparison}\label{sec:nn_comparaison_unic}
