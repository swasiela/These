...
As highlighted in Chapter~\ref{chap:samp}, the computation of such tubes using \myglsentry{odes} integration considerably reduces the efficiency of sampling-based methods, particularly for complex systems.
...
This chapter is organized as follows: Section~\ref{sec:learning_overview} begins with an overview of parallelization techniques and learning methods for solving \myglsentry{odes}.
...
\section{Techniques for accelerating ODEs integration}\label{sec:learning_overview}
...
The set of \myglsentry{odes} that the integration has to be computationally speed-up has the following intrinsic structure within a single time step: Equation~(\ref{eq:ctrl}) computes the control inputs, Equation~(\ref{eq:dyna}) updates the robot state, and Equation~(\ref{eq:dyna_sensi}) computes the sensitivity matrices.
...
Their application is limited because of the intrinsic coupling between different parts of the \myglsentry{odes}, and numerical accuracy. 
...
\subsection{Learning with embedded dynamics}
...
\begin{itemize}
    ...
    Therefore, the proposed approach in this chapter contrasts with Neural \myglsentry{odes}, which aims to learn the system dynamics when \myglsentry{odes} are not explicitly defined.
    ...
    Unlike Neural \myglsentry{odes}, which learn the set of \myglsentry{odes} that govern the system dynamics from data, \myglsentry{pinns} enforce the physical constraints within the network training process itself. 
...
\begin{figure} [t!]
    ...
    \caption{Representation of a \myglsentry{rnn} cell (top left), a \myglsentry{gru} cell (top right), and a \myglsentry{lstm} cell (bottom).
    The three models (RNN, LSTM, and GRU) feature different update mechanisms (blue box). 
    They each take as input a hidden state $h$ (with LSTM also taking a cell state $c$) and a current state from the sequence $x$, and output the updated hidden state as well as the predicted value ${\hat{y}}$.
    }%
    \label{fig:rnns}%
\end{figure}
...
\begin{itemize}
    ...
    Furthermore, using Transformers starting from non-zero initial conditions is not at straightforward as in the \myglsentry{rnns}, where this information is naturally encoded within the hidden state.
...
However, due to the inherent coupling between different parts of the \myglsentry{odes} and the constraints of control applications, these methods are not ideal for the current problem.

The section also explored learning techniques that embed \myglsentry{odes} dynamics directly into the model, such as Neural ODEs and Physics-Informed Neural Networks (PINNs), which are particularly useful when system dynamics are not explicitly known. 
...
They rely on accessing the gradient between the desired predicted outputs and the neural network inputs, a process that can be challenging and computationally intensive. 
This gradient computation often requires additional training effort, as it involves optimizing the network to capture these relationships accurately.

Additionally, learning methods like \gls{rnns} and Transformers, which are well-suited for modeling temporal data, were discussed for their potential in trajectory prediction and dynamic system modeling. 
However, due to the need for explicit temporal dependencies in Transformers, and because \myglsentry{rnns} inherently encode temporal correlations and initial conditions within their hidden state, \myglsentry{rnns} are more suitable for the current application.

Therefore, in this chapter, a \myglsentry{rnn}-based architecture will be explored to directly correlate a desired trajectory with the corresponding uncertainty tubes, eliminating the need to integrate a set of \myglsentry{odes}.
A deeper comparison of different \myglsentry{rnn} cell types will be conducted to choose the most appropriate one for this task.
...
This section presents a multi-task learning neural network based on a GRU architecture, which takes as input the desired robot states and outputs the nominal control inputs $\u_n$, as well as the uncertainty tubes around the states $\Rq$ and the control inputs $\Ru$. 

The resulting predictor, denoted as $\boldsymbol{g}$, aims at bypassing the intrinsic structure of the original set of \myglsentry{odes} considered in this thesis. 
...
Note that each system of equations depends on the outputs of the preceding one within a single time step (i.e., to solve Equation~(\ref{eq:dyna_sensi}), one must first solve Equation~(\ref{eq:dyna})).
...
However, in order to make the learned model independent of workspace boundaries used during planning (i.e. robot position) and initial robot orientations, only the desired linear velocities, accelerations, and yaw angular velocities ($\Dot{\Psi}_d$), are kept as the network input components.
...
Moreover, the control input values $\u = [u_1 ,\, u_2 ,\, u_3 ,\, u_4]^T \in \mathbb{R}^4$,which correspond to the squared propeller speeds, are required for the motion planning algorithm to perform robust feasibility checks, and their computation depends on \myglsentry{odes} forward integration (as shown in Section~\ref{sec:tubes}). 
...
Details of the architecture's implementation are further presented in Section~\ref{sec:training}.
...
Each global trajectory starts from the same initial hovering state ($\q_{init}$) initialized at zero velocities and accelerations, except along the $z$-axis to compensate for gravity.
...
This subsection focuses on identifying the most appropriate \myglsentry{rnns} cell for the proposed architecture, selecting from \myglsentry{rnn}, \myglsentry{gru}, and \myglsentry{lstm}.
...
The results indicate that the average prediction time for neural networks is an order of magnitude faster compared to the \textbf{Euler} method and two orders of magnitude than the \textbf{dopri5} integrator. 
...
In order to confirm this for the neural network as well, an ablation study is conducted on the \textbf{GRU} model to show that the choice of input vector components described in Section~\ref{sec:architecture} is the most relevant for the quadrotor case. 
...
Each model is trained in the same way detailed in Section~\ref{sec:training} and is evaluated according to the MAE metrics described in Section~\ref{sec:metric}.
...
\begin{figure}[h!]
    \centering
    \includesvg[width=0.9\linewidth]{figures/learning_quadrotor/all_models.svg} 
...
The \myglsentry{dfl} controller inputs consist of the desired robot position, linear velocities, and accelerations, as shown in Equation~\ref{eq:unic_control_law}. 
However, to ensure that the learned model is independent of the workspace boundaries used during planning (i.e., the robot's position limits) and since accelerations are not considered for this robot in this thesis, only the desired linear velocities are retained as input components for the network.
Therefore, the input to the neural network for the differential drive robot case is a vector $\boldsymbol{\q_{in}^k} = [\boldsymbol{v_d}]^T \in \mathbb{R}^2$, where $k$ refers to the $k$-th state along the discretized desired trajectory.
...
The dataset is generated using the same method described in Section~\ref{sec:dataset_general}, using the Dubins steering method (see Section~\ref{sec:dubins}) to connect the sampled states together.
...
Before training the neural network, the input features of the dataset (train, val, and test) were min-max scaled according to the velocity magnitude value used when generating the dataset (see Section~\ref{sec:dataset_general}) as it is guaranteed that the velocities components generated cannot exceed this value, ensuring that all velocity values are in the interval $[-v_{magn}, v_{magn}]$.  
The annotations were also normalized using a standard scaling based on the mean and standard deviation values of the training set annotations.
