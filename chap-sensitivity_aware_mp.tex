...
The computational overhead is due to the time spent solving the \myglsentry{odes}, as depicted in Figure~\ref{fig:profiling_unic}, where \myglsentry{sarrt*} and \myglsentry{sasst*} ,that leverage the sensitivity computation, spend approximately 80\% of their total computation time solving the \myglsentry{odes}. 
...
Nevertheless, it is worth noting that the \myglsentry{sasst*} is much faster than the \myglsentry{sarrt*} variant as highlighted in Table~\ref{tab:samp_unicycle} and in Figure~\ref{fig:samp_unic_time} that show how the \myglsentry{sasst*} is able to converge faster than \myglsentry{sarrt*}, highlighting that reducing the frequency of \myglsentry{odes} leads to a lower planning time.
