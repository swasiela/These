...
Now that the new state is globally optimally connected to the tree with a feasible extension, \myglsentry{sarrt*} checks if any optimal rewiring can be performed, as in the standard \myglsentry{rrtstar} implementation, using Algorithm~\ref{alg:RobustRewire}.
...
Therefore, it is preferable to consider the largest eigenvalue of the kernel matrix $\bPi(t) \W \bPi(t)^T$ (see Section~\ref{sec:tubes}) as \emph{sensitivity norm}, since this represents the largest (worst-case) deviation in the state space. 
...
This robustness is also illustrated in Figure~\ref{fig:robust_unic}, where the vanilla \myglsentry{rrtstar} is able to find the minimum cost path solution among the tree which passes through a narrow corridor, however, this solution is not robust to the presence of uncertain parameters, leading to a poor robustness when executed by the controller.
...
It is shown that \myglsentry{sarrt*} and \myglsentry{sasst*}, which rely on sensitivity computation, dedicate about 80\% of their total computing time to solving the \myglsentry{odes}.
...
\begin{figure} [h!]
    ...
    Simulated trajectories under uncertainty are displayed in green in the case of success, and in red in the case of a crash.
    Note that explicitly accounting for high uncertainty, which results in large tubes during planning, does not allow the \myglsentry{sarrt*} to pass through the narrow passage.
        ...
        \While{\textbf{not} StopCondition$(T, {q^{goal}})$}  
            ...
            \If{$Q_{sol} \neq \emptyset$}
                \For { i = 0 and  i < size($Q_{sol}$)-1}
                    ...
                    \State $\left \{\q_n, \u_n, \Rq, \Ru, S_F \right \}  \gets $SolveODEs$(\q_d, S_0);$
                    \If {\textbf{not} IsRobust$(q_n, \u_n, \Rq, \Ru)$}
...
\begin{figure}[t!]
    ...
    The dashed black trajectory (a, b) and the dashed purple cost (bottom) correspond to the \myglsentry{sasst*} sensitivity-optimal trajectory.}
    \label{fig:U_shape}
\end{figure}
...
\begin{figure} [h!]
    \centering
    \subfloat[\centering ]{{\includegraphics[width=0.45\linewidth]{figures/samp/2Ways_low_3in1_before.png} }}%
    \subfloat[\centering ]{{\includegraphics[width=0.45\linewidth]{figures/samp/2Ways_low_3in1_after.png} }}%
    \\
    \subfloat[\centering ]{{\includegraphics[width=0.45\linewidth]{figures/samp/2Ways_high_3in1_before.png} }}%
    \subfloat[\centering ]{{\includegraphics[width=0.45\linewidth]{figures/samp/2Ways_high_3in1_after.png} }}%
