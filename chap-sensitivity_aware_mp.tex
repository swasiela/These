...
However, this robustness comes at the cost of computational efficiency, as highlighted by the significantly higher planning times of the two robust variants when compared to their standard counterparts that focus on optimizing trajectory length.
This is correlated to the time spent solving the \myglsentry{odes}.
Indeed, as depicted in Figure~\ref{fig:profiling_unic}, the two variants relying that rely on sensitivity computation spend approximately 80\% of their total computation time solving the \myglsentry{odes}. 
...
As explained in Section~\ref{sec:unified}, the set of \myglsentry{odes} must be solved at each algorithm iteration at least one per iteration in the case of \myglsentry{sasst*} and tens or hundreds of times for the \myglsentry{sarrt*}.
Even if this leads to high, but still amainable, computation time for simple systems such as the differential drive robots, the gap induced by solving the \myglsentry{odes} grows consequently when applied to more complex systems such as the quadrotor which involves a hundred of \myglsentry{odes}. 
...
Finally, note that in all cases, the rely high planning time of the \myglsentry{sasst*} compared to the lazy approach suggest that the total number of iterations needed to achieve a good state space coverage in order to converge is higher in the context of robust constrained planning, as more iterations must be performed to reach the low cost regions of the space.
