...
% \subsection{Local trajectory generation}\label{sec:forwardplanning}
...
Among the widely used local trajectory planners, Bézier curve-based planners~\cite{cBezier,cBezier2} stand out due to their inherent smoothness and the advantageous convex hull property.
...
Nevertheless, the aforementioned methods focus on generating trajectories within a limited, localized scope according to a partial environment knowledge. 
...
% \subsection{Global motion planning}\label{sec:forwardplanning}
...
However, such local steering techniques may not be available for every systems, as solving the associated \myglsentry{bvp} can be particularly challenging. 
To overcome this, algorithms were developed to bypass this need by performing dynamic system forward propagation instead of solving a complex \myglsentry{bvp}.
...
% % Such local planning methods do not consider obstacle avoidance.
...
% Local planners are essential components in motion planning, focusing on generating feasible motion between two robot states while adhering to various constraints such as kinematic constraints, dynamic constraints, and obstacle avoidance. 
...
% \subsubsection{Global planners}\label{sec:forwardplanning}
...
To address this challenge, an extension of the standard \myglsentry{mpc}, known as \gls{tmpc}, was introduced in~\cite{cTMPC}.
...
It generates a nominal trajectory using \myglsentry{mpc} and defines an invariant ``tub'' around this trajectory, within which the system state must remain. 
