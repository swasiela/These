...
This contribution presents an enhanced version of the sensitivity-aware variants from Chapter~\ref{chap:samp}, integrating the GRU-based architecture from Chapter~\ref{chap:NN} to reduce the computational cost of solving thousands of \myglsentry{odes} in sampling-based contexts. 
...
This chapter is structured as follows: Section~\ref{sec:RASAMP} discusses the integration of the GRU-based architecture into sampling-based planners and outlines the accuracy optimization process.
...
\subsubsection{Sampling-based motion planning with \myglsentry{rnns}}
...
Note that a hidden state $h$ is unique according to its parent, analogous to the initial conditions of \myglsentry{odes} discussed in Chapter~\ref{chap:samp}.
Therefore, its use is only applicable to tree-based planners, where each node has a single parent.
In the remainder of this chapter, it is demonstrated how to integrate this mechanism into the \myglsentry{rrt} algorithm and its optimal variant, \myglsentry{rrtstar}.
...
This subsection proposes a deep learning-based sensitivity-aware variant of the \myglsentry{rrt}~\cite{cRRT}, denoted \gls{deepsarrt} as a particular instance of \myglsentry{deepsamp}, and presented in Algorithm~\ref{alg:ExtensionExample}. 
...
Additionally, note that the standard non-optimal \myglsentry{rrt} is used here, as sensitivity is only employed to enforce robust constraints rather than to find a sensitivity-optimal trajectory, in contrast to the objective of Chapter~\ref{chap:samp}.
...
This decoupling is made possible by the GRU, whereas in \myglsentry{samp} variants, both the control inputs and associated tubes are derived from the full resolution of the \myglsentry{odes}.
...
\begin{algorithm}[h!]
    \caption{\myglsentry{sastomp} [$\q_{d,init}^{i}, N_{roll}$]}\label{alg:sastomp}
    \begin{algorithmic}[1]
...
Therefore, in this quadrotor application, $\bPi \in \mathbb{R}^{13\times6}$, and $\bTheta \in \mathbb{R}^{4\times6}$ resulting in solving 115 \myglsentry{odes} to compute the uncertainty tubes.
...
% (measured on the real robot presented in Section~\ref{sec:Experimental})
...
This section presents comparison between the \myglsentry{deepsamp} variants (DeepSARRT and DeepSARRT*), that leverage the GRU-based neural network, and the \myglsentry{samp} variants that solve the set of \myglsentry{odes}.
...
This time saving is also illustrated by Figure~\ref{fig:NNProfiling}, that shows for both asymptotically optimal and non-optimal sensitivity-aware variants, the contribution of solving the \myglsentry{odes} or performing GRU predictions to the total planning time after 1.000 algorithms iterations.
Note that the \myglsentry{rrtstar} and its sensitivity-aware variant optimize the trajectory time.
...
However, note that the observed time gains differ from those reported in Chapter~\ref{chap:NN} due to certain limitations:
\begin{itemize}
    ...
    However, the proposed \myglsentry{deepsamp} algorithms still require solving the nominal system dynamic through the \myglsentry{odes} of Equation~(\ref{eq:dyna}).
    ...
    This process eliminates the need to solve the system dynamic \myglsentry{odes} (Equation~(\ref{eq:dyna})), and perform robust collision checks for these trajectories.
\end{itemize}
...
One can note the efficiency of the DeepSARRT* that reach approximately one order of magnitude time gain compared to its \myglsentry{sarrt*} counterparts. 
...
\begin{figure} [h!]
    \centering
    \subfloat[\centering Window environment]{{\includegraphics[width=0.3\linewidth]{figures/accuracy/iloveimg-resized/window_pilar.png} }\label{fig:window_pilar}}%
    \subfloat[\centering Corridor environment]{{\includegraphics[width=0.3\linewidth]{figures/accuracy/iloveimg-resized/corridor.png} } \label{fig:corridor}}%
...
\myglsentry{saextendedshct} is evaluated with different hyperparameters chosen as follows: $\delta \in \{0.1, \, 0.01\}$ and $K \in \{1, \, 2, \, 3, \, 5, \, 10\}$.
% Two different sampling strategies are also tested: a uniform sampling that uses the same radius $\delta$ to sample state values in a ball of radius $\delta$ around the current trajectory state, and a Gaussian sampling where $\delta$ is used as the standard deviation.
...
% \begin{figure} [h!]
%     \centering
%     \subfloat[\centering ]{{\includegraphics[width=0.48\linewidth]{figures/accuracy/all_methods_empty.png} }}%
%     \subfloat[\centering ]{{\includegraphics[width=0.48\linewidth]{figures/accuracy/bplot_all_methods_empty.png} } }%
%     \caption{Cost optimization results for the four methods, averaged over 10 plans in a free space environment. 
%     The standard deviation is represented as an envelope around the mean curves.
%     The \myglsentry{sastomp} is shown in red, \myglsentry{SAshortcut} in black, \myglsentry{saextendedshct} in purple, and \myglsentry{cobyla} in blue.}%
%     \label{fig:acc_empty}%
% \end{figure}
...
The \myglsentry{saextendedshct} is tested with a Gaussian sampling using two different standard deviation $\delta \in \{0.1, \, 0.01\}$ and $K = 1$.
...
Therefore, in the following experimental section, the robust local accuracy optimization is performed using \myglsentry{SAshortcut} considering both the trajectory states and controller gains as optimization variables.

Recall that, because the GRU is trained for specific controller gains, and the trajectories encountered during the \myglsentry{SAshortcut} process deviate significantly from those used during training (due to the violation of the maximum local distance during the shortcut process), \myglsentry{SAshortcut} does not benefit from the neural network.
